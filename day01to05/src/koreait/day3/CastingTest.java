...
// 필요한 이유 : 대입, 연산(사칙연산 등)을 실행할때 같은 형식으로 일치를 시켜야 합니다. (자동변환 또는 강제변환)
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		short sval1 = 11;   
		// int 보다 작은 크기의 리터럴은 없습니다. 11은 4바이트, sval1변수는 2바이트
//		sval1 = 40000;   //오류
//		sval1 = sval1 + sva1; // 4바이트로 연산하기 때문에 오류. 강제 캐스팅이 필요합니다.
		
		float fval1 = 1.23f;
		fval1 = fval1 + fval1;  // float으로 연산하기 때문에 정상 실행.
		
		sval1 = (short)40000;   // 강제 캐스팅 -> 데이터 손실 오류(정확한 값이 아님), 큰 자료형을 작은 자료형으로 변환
...
//           : 정수는 4바이트보다 작은 타입은 int 리터럴을 사용하며 표현 범위에서 벗어나면 오류가 발생합니다.
//           : 연산할때 정수는 4바이트보다 작은 타입은 int로 변환된 후에 연산합니다.
